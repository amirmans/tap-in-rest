...
function exitWithHttpError($error_code, $message = '')
{
	...
	{
    //Amir was here
    case ($error_code <= 410): header("HTTP/1.0 400 Bad post values"); break;
    ...
    //TODO: change back to _POST
		if (isset($_REQUEST['cmd']))
		{
			switch (trim($_REQUEST['cmd']))
			{
                ...
                case 'validate': $this->handleValidate(); return;
			}
		}
...
//    $stmt->bindParam(':chatroom', $code);
		$stmt->execute(array($userID, $chatRoomID));
	...
	function handleUpdate()
	{
		...
		$stmt = $this->pdo->prepare('UPDATE chatters SET loggedIn = 0 WHERE udid = ?');
		$stmt->execute(array($udid));
	}
    ...
    function handleValidate()
    {
        try {
            ...
            $stmt = $this->pdo->prepare('SELECT uid FROM chat_businesses_users WHERE businessID = ? and uid = ?');
            $stmt->execute(array($businessID, $userID));
            $rows = $stmt->fetch(PDO::FETCH_OBJ);

            if ($stmt->rowCount() >= 1) {
                $returnVal =  TRUE;
                $result_arr['permission'] = 1;
            }
            else {
                $result_arr['permission'] = 0;
                $returnVal = FALSE;
            }
  ...
  // This method is obselete
	...
	function getUDID()
	{
		if (!isset($_REQUEST['udid']))
			exitWithHttpError(402, 'Missing udid');

		$udid = trim(urldecode($_REQUEST['udid']));
		if (!$this->isValidUDID($udid))
			exitWithHttpError(403, 'Invalid udid');
  ...
  function getUserID()
	{
		if (!isset($_REQUEST['userID']))
			exitWithHttpError(412, 'Missing userID');

		$userID = trim(urldecode($_REQUEST['userID']));
		if (!$this->isValidUserID($userID))
			exitWithHttpError(413, 'Invalid userID');
	...
	function getbusinessID()
	{
		if (!isset($_REQUEST['business_id']))
			exitWithHttpError(412, 'Missing business_id');

		$businessID = trim(urldecode($_REQUEST['business_id']));
	...
	function getDeviceToken($mayBeEmpty = false)
	{
		if (!isset($_REQUEST['token']))
			exitWithHttpError(404, 'Missing device token');
