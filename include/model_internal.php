...
function set_points_map_for_biz_in_market($market, $threshold, $threshold_value) {
    $rt = 0;
	$query = "select merchant_ids from corp where corp_name = $market";
	$corp_result = getDBresult($query);
	$csvString = $corp_result[0]['merchant_ids'];
//	$biz_ids = explode(PHP_EOL, $csvString);
	$biz_array = str_getcsv($csvString);

	$points_level = "trial";
	foreach ($biz_array as $biz) {
        $insert_query = "insert into points_map
			(business_id, points, equivalent, points_level_name) Values ($biz, $threshold, $threshold_value, '$points_level')
			ON DUPLICATE KEY UPDATE points = $threshold, equivalent = $threshold_value;";
		$rt = insertOrUpdateQuery($insert_query);
	}
...
//                break 2;
		...
		case 'set_points_map_for_biz_in_market':
			$pos = stripos($cmd, "set_points_map_for_biz_in_market");
			if ($pos !== false) {
				$market = filter_input(INPUT_GET, 'market_name');
                $threshold = filter_input(INPUT_GET, 'threshold');
                $threshold_value = filter_input(INPUT_GET, 'threshold_value');
				$result = set_points_map_for_biz_in_market($market,$threshold,$threshold_value);
				echo json_encode($result);
			}
			break;
