...
function save_order($business_id, $customer_id/*, $total*/, $subtotal, $tip_amount, $points_dollar_amount
	...
	if ($corp_id > 0) {
		$dateArr = array();
		$corp_query = "select delivery_week_days, cutoff_no_days from corp where corp_id = $corp_id;";
		$row_corp = getDBresult($corp_query);
		calc_pickup_cutoff_date($dateArr, $row_corp[0]['delivery_week_days'],$row_corp[0]['cutoff_no_days']);
		$pickup_date = $dateArr['pickup_date'];
	}
	...
	$insert_query = "insert into `order` (business_id, consumer_id, total, subtotal, tip_amount,
	  ...
	  , pd_time, pd_instruction, order_type, order_corp_id, pickup_date)
	Values ($business_id, $customer_id, $total, $subtotal, $tip_amount, $points_dollar_amount, $tax_amount,
	  ...
	  , '$pd_time', '$pd_instruction', '$order_type', '$corp_id', '$pickup_date');";
...
function get_global_points_for_customer($consumerID)
{
  ...
  and consumer_id = '$consumerID' and points_reason_id = 5;";
	...
	if (empty($result[0]["sum_global_points"])) {
		$result[0]["sum_global_points"] =0;
	}
